...
public class Game2048 extends JPanel {
	public Square[][] array = new Square[4][4];
	// public ArrayList<ArrayList<Square>> array;
	Random r = new Random();
	...
	public void Lose(){
		...
		for(int i=0;i<4;i++){
			for(int j=0;j<4;j++){
				...
				if(array[i][j].number==array[i][j-1].number&&j!=0){
					loser=false;
				}
				if(array[i][j].number==array[i+1][j].number&&i!=3){
					loser=false;
				}
				if(array[i][j].number==array[i-1][j+1].number&&j!=0){
					loser=false;
				}
			}
		}
